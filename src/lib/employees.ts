...
export async function getEmpleadosBaja(): Promise<EmpleadoBajaItem[]> {
  let conteo: { nomina: string; totalBienes: number }[];
  try {
    const { data } = await apiFetch<{ data: { nomina: string; totalBienes: number }[] }>(
      "/api/activos/conteo-por-nomina"
    );
    conteo = data;
  } catch {
    return [];
  ...
  for (const row of conteo) countMap.set(row.nomina, row.totalBienes);
