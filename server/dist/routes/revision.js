...
router.put("/:id", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const { rows: existing } = await db_1.pool.query("SELECT id, estatus FROM revision_sesiones WHERE id = $1", [id]);
        const isNew = existing.length === 0;
        await db_1.pool.query(`INSERT INTO revision_sesiones
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         estatus = EXCLUDED.estatus`, [id, id_usuario, mode ?? "responsable", responsable_id ?? "", responsable_nombre ?? "", responsable_tipo ?? "", JSON.stringify(expected ?? []), JSON.stringify(scans ?? []), notes ?? null, estatus ?? "activa"]);
        if (isNew) {
            await auditLog("revisiones", id, "CREATE", null, null, responsable_nombre ?? null, req.user, req.ip);
        }
        res.json({ data: null });
    }
    catch (err) {
...
router.patch("/:id/finalizar", auth_1.requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.pool.query("UPDATE revision_sesiones SET estatus = 'finalizada' WHERE id = $1", [id]);
        await auditLog("revisiones", id, "FINALIZAR", null, null, null, req.user, req.ip);
        res.json({ data: null });
    }
    catch (err) {
...
router.get("/activos-info", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const { rows } = await db_1.pool.query("SELECT numero_inventario, descripcion, marca, modelo, numero_serie, ultimo_nomina FROM activos WHERE numero_inventario = ANY($1)", [numeros]);
        res.json({ data: rows });
    }
    catch (err) {
