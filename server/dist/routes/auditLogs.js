...
router.get("/", auth_1.requireAuth, async (req, res) => {
    try {
        const { tabla, registroId, accion, usuario, fechaDesde, fechaHasta, limit = "50", offset = "0", } = req.query;
        const params = [];
        const conditions = [];
        let i = 1;
        if (tabla && tabla !== "todos") {
            conditions.push(`al.tabla = $${i++}`);
            params.push(tabla);
        }
        if (registroId) {
            conditions.push(`al.registro_id = $${i++}`);
            params.push(registroId);
        }
        if (accion && accion !== "TODOS") {
            conditions.push(`al.accion = $${i++}`);
            params.push(accion);
        }
        if (usuario) {
        ...
        if (fechaHasta) {
            conditions.push(`al.created_at <= $${i++}`);
            params.push(fechaHasta + "T23:59:59Z");
        }
        const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
        const countResult = await db_1.pool.query(`SELECT COUNT(*) FROM audit_logs al ${where}`, params);
        const total = parseInt(countResult.rows[0].count);
        params.push(parseInt(limit) || 50, parseInt(offset) || 0);
        const { rows } = await db_1.pool.query(`SELECT al.id, al.tabla, al.registro_id, al.accion, al.campo,
