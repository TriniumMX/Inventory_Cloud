...
router.get("/modulos", auth_1.requireAuth, async (req, res) => {
    if (!isSuperAdmin(req, res))
        return;
    try {
        const { rows } = await db_1.pool.query("SELECT id_modulo, clave, nombre, grupo, orden FROM modulos ORDER BY grupo, orden");
        res.json({ data: rows });
    }
    catch (err) {
...
router.get("/usuario/:id", auth_1.requireAuth, async (req, res) => {
    if (!isSuperAdmin(req, res))
        return;
    try {
        const userId = parseInt(req.params.id, 10);
        const { rows } = await db_1.pool.query(`SELECT m.clave,
...
router.put("/usuario/:id", auth_1.requireAuth, async (req, res) => {
    if (!isSuperAdmin(req, res))
        return;
    const userId = parseInt(req.params.id, 10);
    ...
    try {
        ...
        for (const m of modulos) {
            if (!m.puedeVer && !m.puedeEditar)
                continue;
            const { rows } = await client.query("SELECT id_modulo FROM modulos WHERE clave = $1", [m.clave]);
            if (rows.length === 0)
                continue;
            await client.query(`INSERT INTO usuario_modulos (id_usuario, id_modulo, puede_ver, puede_editar)
         VALUES ($1, $2, $3, $4)`, [userId, rows[0].id_modulo, m.puedeVer, m.puedeEditar ?? false]);
        }
